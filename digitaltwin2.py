...
from astroquery.vizier import Vizier # Changed back to Vizier
from astroquery.exceptions import TimeoutError # Removed NoResultFound
...
# --- Function to fetch stars from Hipparcos (ICRS) via VizieR ---

def fetch_stars_from_hipparcos(ra_center_deg, dec_center_deg, radius_deg, magnitude_limit=7.0, max_stars=100):
    """
    Fetches stars from the Hipparcos Main Catalogue (I/239/hip_main) using astroquery.vizier.
    Queries a circular region around a given RA/Dec.
    """
    print(f"\nAttempting to fetch stars from Hipparcos (ICRS) around RA={ra_center_deg}°, Dec={dec_center_deg}° with radius={radius_deg}°...")
    try:
        # Configure VizieR to query the Hipparcos Main Catalogue
        # 'I/239/hip_main' is the VizieR ID for the Hipparcos Main Catalogue
        # We request specific columns: HIP (star ID), _RAJ2000 (RA in ICRS), _DEJ2000 (Dec in ICRS), Vmag (Visual Magnitude)
        vizier = Vizier(columns=["HIP", "_RAJ2000", "_DEJ2000", "Vmag"], catalog=["I/239/hip_main"])
        
        # Set a row limit to avoid fetching excessively large catalogs for a demo
        vizier.ROW_LIMIT = max_stars 

        # Define the coordinate for the query
        ...
        # Removed: if not tables: raise NoResultFound(...)
        # The try-except block below will catch if tables is empty or if there's an issue accessing it.

        hipparcos_table = tables[0] # This will raise an IndexError if tables is empty.

        stars = []
        for row in hipparcos_table:
            # Filter by magnitude
            if row['Vmag'] < magnitude_limit:
                star_id = row['HIP']
                ra = row['_RAJ2000']
                dec = row['_DEJ2000']
                magnitude = row['Vmag']
        ...
        print(f"Successfully fetched {len(stars)} stars from Hipparcos.")
        return stars

    except (TimeoutError, Exception) as e: # Catching general Exception now
        print(f"Failed to fetch stars from Hipparcos API: {e}")
...
if __name__ == "__main__":
    ...
    # Attempt to fetch real stars from Hipparcos (ICRS)
    real_stars_data = fetch_stars_from_hipparcos( # Changed back to fetch_stars_from_hipparcos
    ...
    # from the fetched Hipparcos stars, but are kept consistent with the original mock
