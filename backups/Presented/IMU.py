...
Older_AddedQuaternionOffset = (0, 0, 0, 0) 
...
CALIBRATION_DRIFT_TIME = 15 # seconds
def AddOffsetToQuaternion(quaternion):
    global AddedQuaternionOffset, Older_AddedQuaternionOffset
    print_info(f"added calibration {quaternion}")
    Older_AddedQuaternionOffset = AddedQuaternionOffset
    AddedQuaternionOffset = quaternion

def GetLastAddedOffset():
    global AddedQuaternionOffset, Older_AddedQuaternionOffset
    return AddedQuaternionOffset, Older_AddedQuaternionOffset


def getLastCalibration():
    global lastCalibrationTimestamp
    return lastCalibrationTimestamp

def is_IMU_calibrated_recently():
    global lastCalibrationTimestamp, CALIBRATION_DRIFT_TIME
    return (time.time() - lastCalibrationTimestamp <= CALIBRATION_DRIFT_TIME)
...
def DetectJitter(quaternion, threshold=0.25):
    ...
    if LastQuaternion == (1, 0, 0, 0): # reset'ed quaternion
        LastQuaternion = quaternion
        return quaternion
    ...
    if diff > threshold:  # threshold for jitter detection
        ...
        bugDetected = True
        # rerun to see if problems are present
        new_dif = differenceInQuaternions(quaternion, IMU_Quaternion_After_Proceessing())
        if(new_dif <= threshold and diff != new_dif ):
            LastQuaternion = quaternion
...
def IMU_Quaternion_After_Proceessing():
    if not is_IMU_available():
        return (1,0,0,0)
...
def read_quaternion():
    ...
    camera_matching = (0.924,0,0,-0.383) # to match camera 
    new_quaternion = multiply_quaternions(new_quaternion,camera_matching)
...
def ResetToCalibration():
    global InvertedQuaternion, InCalibration, LastQuaternion, lastCalibrationTimestamp
    InCalibration = True
    print("Starting software calibration of IMU")
    

    q = read_quaternion()
    # print("ended quaternion read")
    ...
    # print("ended quaternion read")
    lastCalibrationTimestamp = time.time()
    # print("ended quaternion read")
...
def test_IMU():
    ...
    try:
        ...
        if accel_ok and mag_ok and gyro_ok and mcu_ok and (sys_status == 5 or sys_status == 0 or sys_status == 1 or sys_status == 133 or sys_status == 128):
...
def InitIMU():
    ...
    for i in range(MAX_REBOOT_ATTEMPTS):
        ...
        print_info("rotating axises")
        # Default mapping (X=X, Y=Y, Z=Z)
        bus.write_byte_data(IMU_I2C_ADDR, AXIS_MAP_CONFIG, 0x24)  
        # Flip X and Y (bits 2 and 1 set)
        bus.write_byte_data(IMU_I2C_ADDR, AXIS_MAP_SIGN, 0x06)   
