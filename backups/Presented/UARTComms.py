...
def listen_for_command():
    ...
    """
    print_info("Waiting for commands from UART")
    while True:
...
def _send_quaternion_response():
    ...
    try:
        ...
        if final_quaternion is None:
            print_error("Returned quaternion is empty. Check system for problems")
            return

        trust_bits = int(trust_factor) & 0b11
        (w, x, y, z) = round_quaternion(final_quaternion, 3)
        payload = struct.pack('<4fB', w, x, y, z, trust_bits)

        checksum = sum(payload) & 0xFF

        packet = b'\xAA' + payload + struct.pack('<B', checksum)

        print(f"Quaternion: {w}, {x}, {y}, {z} | Trust: {trust_factor}")
        print(f"Packet ({len(packet)} bytes): {packet}")
...
def _handle_command(command):
    ...
    if command == "<PING>":
        ...
        data = f"-{get_current_lis_state()}-\n"
        serial_object.write(data.encode('utf-8'))
    else:
        # Silently ignore unknown commands
        print_error(f"Unknown command from UART {command}")
        pass
