...
TOLERANCE = 2
IMAGE_FILE = "./test_images/Capela-02.png"
NUM_STARS = 10
EPSILON = 1e-6
MIN_MATCHES = 5
...
def lost_in_space(picture_path = None):
    using_image_path=None
    if picture_path is not None:
        if os.path.exists(picture_path):
            using_image_path = picture_path
        else:
            print_fatal_error(f"Captured image {picture_path} not found.")
            return
    else:
        using_image_path = IMAGE_FILE  # Ensure IMAGE_FILE is defined elsewhere

    # Now you can proceed using `using_image_path`
    print(f"Using image: {using_image_path}")

    star_coords = find_brightest_stars(using_image_path, NUM_STARS)
    display_star_detections(using_image_path, star_coords)
